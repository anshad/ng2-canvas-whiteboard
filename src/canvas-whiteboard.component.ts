import { Component, Input, Output, EventEmitter, ViewChild, ElementRef, OnInit, OnChanges, OnDestroy, AfterViewInit, NgZone, ChangeDetectorRef } from '@angular/core';
...
export class CanvasWhiteboardComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  private _initCanvasServiceObservables(): void {
    this._canvasWhiteboardServiceSubscriptions.push(this._canvasWhiteboardService.canvasDrawSubject$.subscribe(updates => this.drawUpdates(updates)));
    this._canvasWhiteboardServiceSubscriptions.push(this._canvasWhiteboardService.canvasClearSubject$.subscribe(() => this.clearCanvas()));
    this._canvasWhiteboardServiceSubscriptions.push(this._canvasWhiteboardService.canvasUndoSubject$.subscribe(updateUUD => this._undoCanvas(updateUUD)));
    this._canvasWhiteboardServiceSubscriptions.push(this._canvasWhiteboardService.canvasRedoSubject$.subscribe(updateUUD => this._redoCanvas(updateUUD)));
  ...
  private _getCanvasEventPosition(eventData: any): CanvasWhiteboardPoint {
    ...
    if (!hasTouches) hasTouches = eventData.changedTouches && eventData.changedTouches.length ? eventData.changedTouches[0] : null;
  ...
  downloadCanvasImage(returnedDataType: string = 'image/png', downloadData?: string | Blob, customFileName?: string): void {
    if (window.navigator.msSaveOrOpenBlob === undefined) {
      ...
      let fileName = customFileName ? customFileName : this.downloadedFileName ? this.downloadedFileName : 'canvas_drawing_' + new Date().valueOf();
